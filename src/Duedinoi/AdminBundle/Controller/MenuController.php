...
use Duedinoi\AdminBundle\Entity\Menu;
use Duedinoi\AdminBundle\Form\MenuType;
use Duedinoi\AdminBundle\Entity\MenuItem;
use Duedinoi\AdminBundle\Form\MenuItemType;
use Duedinoi\AdminBundle\Form\MenuItemPageType;
...
class MenuController extends Controller {
    ...
    public function indexAction() {
        ...
        return $this->render('DuedinoiAdminBundle:Menu:index.html.twig', array(
                    'entities' => $pagination,
    ...
    public function showAction($id) {
        ...
        return $this->render('DuedinoiAdminBundle:Menu:show.html.twig', array(
                    'entity' => $entity,
    ...
    public function editAction($id) {
        ...
        $menuItems = $em->getRepository('DuedinoiAdminBundle:MenuItem')->findBy(array('menu'=>$id ));

        $editForm = $this->createEditForm($entity);
        $deleteForm = $this->createDeleteForm($id);

        return $this->render('DuedinoiAdminBundle:Menu:edit.html.twig', array(
                    'entity' => $entity,
...
//                    );
    ...
    public function menuItemAction($menu_id, Request $request) {
        ...
        $menu = $em->getRepository('DuedinoiAdminBundle:Menu')->find($menu_id);
        if (!$menu) {
        ...
        $items = $em->getRepository('DuedinoiAdminBundle:MenuItem')->findBy(array('menu' => $menu));
        ...
        return $this->render('DuedinoiAdminBundle:Menu:menu_items.html.twig', array(
                    'form' => $form->createView(),
                    'items' => $items,
                    'menu' => $menu
        ));
    ...
    private function setPosition($em, $hierarchy, $parentId = null) {
        foreach ($hierarchy as $newPosition => $vars) {
            ...
            $menuItem = $em->getRepository('DuedinoiAdminBundle:MenuItem')->find($vars['id']);
            $menuItem->setPosition($newPosition);

            if (!is_null($parentId)) {
                $menuItemParent = $em->getRepository('DuedinoiAdminBundle:MenuItem')->find($parentId);
    ...
    public function addMenuItemAction($id, Request $request)
    {
        if($request->isMethod('post')) {
            ...
            $em = $this->getDoctrine()->getManager();
            $menu = $em->getRepository('DuedinoiAdminBundle:Menu')->find($id);
            if(!$menu) {
    ...
    public function removeMenuItemAction(Request $request)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $menuItem = $em->getRepository('DuedinoiAdminBundle:MenuItem')->find($menuitem_id);
    ...
    public function getEditModalAction(Request $request)
    {
        ...
        $menuItem = $em->getRepository('DuedinoiAdminBundle:MenuItem')->find($menuItem);
        
        $title = 'Edit '. $menuItem->getTitle(). ' item';
        
        $form = $this->createForm(new \Duedinoi\AdminBundle\Form\MenuItemPageType(), $menuItem);
        
        return new \Symfony\Component\HttpFoundation\JsonResponse(array(
            'content' => $this->renderView('DuedinoiAdminBundle:Menu:menu_item_modal.html.twig', array(
                            'menu_item' => $menuItem,
                            'title' => $title,
                            'form' => $form->createView()
                        ))
