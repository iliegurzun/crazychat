 ...
 * @author Ilie
 */
namespace Duedinoi\AdminBundle\Extension;
...
//        'child_age' => new \Twig_Filter_Method($this, 'getChildAge'),
  ...
  public function getDefaultSeoTitle()
  {
      ...
      $setting = $dm->getRepository('DuedinoiAdminBundle:Setting')->findOneBy(array('name' => 'default_seo_title'));
...
//            $output->writeln($e->getMessage());
    ...
    public function getAdminMenuSortable($menu, $id = null, $frontend = false)
    {
        ...
        if(is_string($menu)) {
            ...
            $menu = $em->getRepository('DuedinoiAdminBundle:Menu')->findOneByName($menu);
        }
...
//              $element->setAttribute('class', 'has-submenu');
        ...
        if($first == true) {
            ...
            if($menuItem->getPage()) {
                $url = $this->service_container->get('router')->generate('Duedinoi_web_default', array('path' => $menuItem->getPage()->getPath()));
    ...
    {
        $asset = $this->service_container->get('templating.helper.assets');
        $userImage = '/bundles/duedinoiweb/images/duedinoi-hero-image-crop.jpg';
        if ($user->hasProfilePicture()) {
            $userImage = $user->getProfilePicture()->getWebPath();
        }
