...
//		!verboseBuild || console.log('            proton.dashboard.lightUp()');
	...
	loadWidgetPositions : function () {
		...
		var positionsFromCookie = false; //$.cookie('proton_widgetPositions') || false;
...
//		!verboseBuild || console.log('            proton.dashboard.widgetSort()');
		...
		$( ".widget-group" ).sortable({
		    ...
		    start: function(event, ui) {
		        proton.dashboard.isDragActive = true;
		        $('.tooltip').tooltip('hide');
                        $( ".widget-group").children('div').on('mouseup', function() {
                            $(this).unbind('mouseup');
                              updatePositions($( ".widget-group").data('sortable-url'), $(".widget-group").sortable('serialize'));
                        });
                        $( ".widget-group > div").on('click', function(e) {
                           e.preventDefault();
                           e.stopPropagation();
                        });
		    },
...
//		!verboseBuild || console.log('            proton.dashboard.setBlankWidgets()');
		...
		var newPlaceholderNum = (widgetRows * widgetsPerRow) - realWidgetNum - 1;
...
var updatePositions = function(url, string) {
    $.ajax({
        url: url,
        data: {
            order: string
