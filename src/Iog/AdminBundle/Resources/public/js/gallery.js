...
//          var is_safari = navigator.userAgent.indexOf("Safari") > -1;
          ...
          if (file.error) {
              ...
              node.delay(3000).fadeOut(function() { data.abort(); removeFromFileUploadData(data); });
          }
          ...
          $.each(data.result.files, function (index, file) {
              if (file.saved == true) {
                 ...
                 // console.log($("#images-area li p.no-img").length);
...
var removeFromFileUploadData = function(data) {
  if(fileUploadData.length == 0) return;
...
var removeFileUploadDataItem = function(pos) {
  if(pos > fileUploadData.length - 1) return;
